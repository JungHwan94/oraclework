...
-- 전직원의 평균급여보다 더 많은 급여를 받는 사원의 사번, 사원명, 직급코드, 급여조회
...
/*
    ...
    - 단일행 서브쿼리: 서브쿼리를 실행한 결과 오로지 1개일때(한행 한열)
    - 다중행 서브쿼리: 서브쿼리를 실행한 결과 여러행 일때(여러행 한열)
    - 다중열 서브쿼리: 서브쿼리를 실행한 결과 여러행 일때(한행 여러열)
    - 다중행 다중열 서브쿼리: 서브쿼리를 실행한 결과 여러행 여러열일때(여러행 여러열)
    
    >> 서브쿼리의 종류가 무엇이냐에 따라 서브쿼리에 앞에 붙는 연산자가 달라짐
*/
...
/*
    ...
    =, !=, >, <, <>...
*/
-- 전직원의 평균급여보다 급여를 더 적게 받는 사원들의 사원명, 직급코드, 급여조회
...
/*
    ...
    - > ANY 서브쿼리: 여러개의 결과값 중 '한개라도' 클 경우
                    즉, 결과값 중 가장 작은값보다 클 경우
    - < ANY 서브쿼리: 여러개의 결과값 중 '한개라도' 작은 경우
                    즉, 결과값 중 가장 큰값보다 작을 경우
*/
...
-- 대리 직급임에도 과장의 급여보다 최소 급여보다 많이 받는 직원의 사번, 사원명, 직급명, 급여조회
SELECT SALARY
FROM EMPLOYEE
...
-- 차장 직급임에도 과장직급의 급여보다 적게 받는 사람의 사번, 사원명, 직급명, 급여조회
...
-- 차장의 가장 큰 급여보다 더 많이 받는 과장 사번, 사원명, 직급명, 급여조회
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '과장'
...
AND E.JOB_CODE = J.JOB_CODE
AND (CASE 
           WHEN SUBSTR(E.EMP_NO, 1, 2) <= TO_CHAR(SYSDATE, 'YY') 
           THEN 2000 + TO_NUMBER(SUBSTR(E.EMP_NO, 1, 2)) 
           ELSE 1900 + TO_NUMBER(SUBSTR(E.EMP_NO, 1, 2)) 
        END) =
    (SELECT MAX(CASE 
                   WHEN SUBSTR(EMP_NO, 1, 2) <= TO_CHAR(SYSDATE, 'YY') 
                   THEN 2000 + TO_NUMBER(SUBSTR(EMP_NO, 1, 2)) 
...
SELECT EMP_NAME, DEPT_ID, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE E
...
SELECT EMP_NAME, J.JOB_CODE, JOB_NAME, 
       SALARY * (1 + NVL(BONUS, 0)) AS TOTAL_SALARY
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE
AND E.BONUS IS NULL
...
SELECT D.DEPT_TITLE, SUM(E.SALARY) AS 부서별_급여합계
FROM EMPLOYEE E
JOIN DEPARTMENT D ON E.DEPT_CODE = D.DEPT_ID
GROUP BY D.DEPT_TITLE
HAVING SUM(E.SALARY) > (SELECT SUM(SALARY) * 0.2 FROM EMPLOYEE);
-- 12-2. 인라인 뷰 사용
SELECT DEPT_TITLE, 부서별_급여합계
FROM (SELECT D.DEPT_TITLE, SUM(E.SALARY) AS 부서별_급여합계
      FROM EMPLOYEE E
      JOIN DEPARTMENT D ON E.DEPT_CODE = D.DEPT_ID
      GROUP BY D.DEPT_TITLE)
WHERE 부서별_급여합계 > (SELECT SUM(SALARY) * 0.2 FROM EMPLOYEE);
-- 12-3. WITH 사용
WITH 부서급여 AS (
    SELECT D.DEPT_TITLE, SUM(E.SALARY) AS 부서별_급여합계
    FROM EMPLOYEE E
    ...
    SELECT SUM(SALARY) * 0.2 AS 전체_급여_20 FROM EMPLOYEE
)
SELECT B.DEPT_TITLE, B.부서별_급여합계
FROM 부서급여 B, 전체급여 T
WHERE B.부서별_급여합계 > T.전체_급여_20;
